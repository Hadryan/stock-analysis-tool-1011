...
def calculate_beta(stock, ind):
    ...
    for date, row in inddf.iterrows():
        stock.loc[date, 'Close Price of SP500'] = row['Close Price of SP500']
        stock.loc[date, '% Return of SP500'] = row['% Return of SP500']
...
def add_risk_free_column(stock, riskrates):
    ...
    resdf = riskrates[riskrates.Date.between(
        stock.iloc[-1]['Date'], stock.iloc[0]['Date'])]
    resdf = resdf.set_index("Date")
    stock = stock.set_index("Date")

    for date, row in resdf.iterrows():
        stock.loc[date, 'Rate'] = row['Rate']
...
def calculate_alpha(stock, ind):
    ...
    for date, row in ind.iterrows():
        stock.loc[date, '% YTD of SP500'] = row['% YTD of SP500']
...
def close_price_as_percent_of_LV_HV_BA(stock):
    ...
    for b in bands:
        ...
        for i, row in stock.iterrows():
            ...
            stock.loc[specific_dates.index, bcols] = [
...
def perform_operation(security_code):
    try:
        ...
        stock_df.to_csv(os.path.join(path, "GRStock/"+"gr" +
